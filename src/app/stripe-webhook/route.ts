...
export async function POST(req: Request) {
  try {
    ...
    if (event.type === 'checkout.session.completed' || event.type === 'checkout.session.async_payment_succeeded') {
      ...
      if (!metadata || !metadata.userId || !metadata.serviceId || !metadata.timeSlotId) {
      ...
      if (!process.env.POCKETBASE_ADMIN_EMAIL || !process.env.POCKETBASE_ADMIN_PASSWORD) {
        console.error('Отсутствуют учетные данные администратора PocketBase в ENV.');
      ...
      console.log(`✅ Бронирование создано для пользователя ${metadata.userId} и слота ${metadata.timeSlotId}`);
    }
